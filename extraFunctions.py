...
def findStationLines(stationsMapDictionary):
    
    '''
    This fucntion retuns all the lines each station has
    It needs the stationsMapDictionary created before to run
    the retun is like:
    {
        "station name" : [lines that are in that station]
...
def Exercise4Tester():
    ...
    '''
    This fucntion has been built to test the exercise 4 fucntion
    It creates an array with all the stations
    then selects two random variables and calls Exercise 4

    It does in loop for as many times as the user wants

    Output:
    The number of times an error occured (exercise 4 returned false)
    ...
    try:
        ...
        for i in range (0, simulationNumber):
            ...
            result  = main.Exercise4(start, end)
            if(not result) : 
